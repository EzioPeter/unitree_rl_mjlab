...
from scripts.reinforcement_learning.rwm_trace.selection import (
    command_region_stratified_top_alpha,
    command_stratified_top_alpha,
    global_top_alpha,
)
...
def test_prompt_contains_go2_order_and_exact_label_contract() -> None:
    ...
    pair = {
        "pair_id": "p",
        "comparison_type": "same_start_candidate",
        "trajectory_i": left,
        "trajectory_j": right,
    }
    prompt = build_go2_feedback_prompt(pair)
    assert "commanded base velocity tracking" in prompt
...
def test_fixed_pair_quota_does_not_use_scores() -> None:
    rows = [
        summary("a0", start="a"),
        summary("a1", start="a"),
    ...
    pairs = build_feedback_pairs(
        rows,
        quota=PairQuota(within_region=1, cross_region=1),
        pair_prefix="test",
        seed=7,
    ...
    assert len(pairs) == 2
    assert sum(
        row["comparison_type"] == "within_command_region" for row in pairs
    ) == 1
    assert sum(
        row["comparison_type"] == "cross_command_region" for row in pairs
    ) == 1
    assert all("pair_score_gap" not in row for row in pairs)


def test_six_command_regions_use_normalized_diagonal_boundaries() -> None:
    scales = (0.5, 0.2)
    cases = (
        ((0.3, 0.05, 0.0), "front"),
        ((-0.3, 0.05, 0.0), "back"),
        ((0.05, 0.15, 0.0), "left"),
        ((0.05, -0.15, 0.0), "right"),
        ((0.0, 0.0, 0.3), "pure_yaw"),
        ((0.0, 0.0, 0.0), "stand"),
        # On y=x in normalized coordinates, the deterministic tie goes front.
        ((0.25, 0.1, 0.0), "front"),
    )
    for index, (command, expected) in enumerate(cases):
        row = summary(f"region-{index}", start=f"s{index}", command=command)
        assert command_region(row, planar_command_scales=scales) == expected


def test_region_pairing_exact_80_20_and_seed_reproducible() -> None:
    rows = [
        *[
            summary(f"front-{index}", start=f"f{index}", command=(0.4, 0.02, 0.0))
            for index in range(10)
        ],
        *[
            summary(f"back-{index}", start=f"b{index}", command=(-0.4, 0.02, 0.0))
            for index in range(10)
        ],
    ]
    kwargs = {
    ...
    first = build_feedback_pairs(rows, **kwargs)
    second = build_feedback_pairs(rows, **kwargs)
    assert [
        (row["trajectory_i"]["trajectory_id"], row["trajectory_j"]["trajectory_id"])
        for row in first
    ] == [
        (row["trajectory_i"]["trajectory_id"], row["trajectory_j"]["trajectory_id"])
        for row in second
    ...
    assert sum(
        row["comparison_type"] == "within_command_region" for row in first
    ) == 8
    assert sum(
        row["comparison_type"] == "cross_command_region" for row in first
    ) == 2
...
def test_command_stratified_top_alpha_preserves_each_present_mode() -> None:
    rows = [
        summary("x0", start="0", command=(0.5, 0.0, 0.0)),
        summary("x1", start="1", command=(0.5, 0.0, 0.0)),
        summary("x2", start="2", command=(0.5, 0.0, 0.0)),
        summary("x3", start="3", command=(0.5, 0.0, 0.0)),
        summary("y0", start="4", command=(0.0, 0.1, 0.0)),
        summary("y1", start="5", command=(0.0, 0.1, 0.0)),
    ]
    result = command_stratified_top_alpha(
        rows,
        [10.0, 9.0, 8.0, 7.0, 0.2, 0.1],
        alpha=0.25,
        seed=3,
    )
    assert result.selected_indices == (0, 4)
    assert result.selected_count == 2


def test_command_region_stratified_top_alpha_preserves_each_present_region() -> None:
    rows = [
        summary("front-low", start="f0", command=(0.4, 0.0, 0.0)),
        summary("front-high", start="f1", command=(0.4, 0.0, 0.0)),
        summary("left-low", start="l0", command=(0.0, 0.15, 0.0)),
        summary("left-high", start="l1", command=(0.0, 0.15, 0.0)),
    ]
    result = command_region_stratified_top_alpha(
        rows,
        [0.0, 1.0, 0.0, 1.0],
        alpha=0.1,
        seed=0,
    )
    assert set(result.selected_indices) == {1, 3}
    assert result.selected_count == 2
...
def test_mutable_buffer_command_region_sim_ratios_are_exact() -> None:
    ratios = {
        "front": 0.05,
        "back": 0.15,
        "left": 0.05,
    ...
    buffer = MutableTraceReplayBuffer(
        100,
        seed=4,
        command_region_sim_ratios=ratios,
        command_active_thresholds=(0.03, 0.02, 0.03),
    )
    front_trace = replay_batch(20, 3.0)
    front_trace["observation"][:, 9:12] = torch.tensor([0.5, 0.0, 0.0])
    back_trace = replay_batch(20, 4.0)
    back_trace["observation"][:, 9:12] = torch.tensor([-0.3, 0.0, 0.0])
    buffer.add_batches(
        [front_trace, back_trace],
        trajectory_ids=["front", "back"],
        scores=[1.0, 1.0],
        refresh_step=1,
    )
    rwm = replay_batch(200, 2.0)
    rwm["observation"][:100, 9:12] = torch.tensor([0.5, 0.0, 0.0])
    rwm["observation"][100:, 9:12] = torch.tensor([-0.3, 0.0, 0.0])
    mixed, sim_mask, report = buffer.mix_rwm_by_command_region(
        rwm,
        generator=torch.Generator().manual_seed(8),
        device="cpu",
    )
    assert int(sim_mask.sum()) == 20
    assert int((mixed["reward"] == 3.0).sum()) == 5
    assert int((mixed["reward"] == 4.0).sum()) == 15
    assert report["front_sim_count"] == 5
    assert report["back_sim_count"] == 15
    assert report["front_sim_shortfall"] == 0
    assert report["back_sim_shortfall"] == 0


def test_command_region_index_cache_tracks_fifo_wrap_and_restore() -> None:
    ratios = {region: 1.0 for region in COMMAND_REGIONS}
    buffer = MutableTraceReplayBuffer(
        5, seed=4, command_region_sim_ratios=ratios
    )
    first = replay_batch(5, 1.0)
    first["observation"][:, 9:12] = torch.tensor([0.5, 0.0, 0.0])
    buffer.add_batch(
        first, trajectory_id="front", learned_score=1.0, refresh_step=1
    )
    replacement = replay_batch(3, 7.0)
    replacement["observation"][:, 9:12] = torch.tensor([-0.3, 0.0, 0.0])
    buffer.add_batch(
        replacement, trajectory_id="back", learned_score=2.0, refresh_step=2
    )

    restored = MutableTraceReplayBuffer(
        5, seed=4, command_region_sim_ratios=ratios
    )
    restored.load_state_dict(buffer.state_dict())
    rwm = replay_batch(4, 2.0)
    rwm["observation"][:2, 9:12] = torch.tensor([0.5, 0.0, 0.0])
    rwm["observation"][2:, 9:12] = torch.tensor([-0.3, 0.0, 0.0])
    mixed, _, report = restored.mix_rwm_by_command_region(
        rwm,
        generator=torch.Generator().manual_seed(8),
        device="cpu",
    )
    assert report["front_sim_count"] == 2
    assert report["back_sim_count"] == 2
    assert int((mixed["reward"] == 1.0).sum()) == 2
    assert int((mixed["reward"] == 7.0).sum()) == 2
