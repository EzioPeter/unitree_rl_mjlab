...
// MuJoCo publishes the simulated base frame velocity through
// rt/sportmodestate. The message is in the world frame, while the policy
// expects base_lin_vel expressed in the body frame.
REGISTER_OBSERVATION(base_lin_vel)
{
    static auto highstate =
        std::make_shared<unitree::robot::go2::subscription::SportModeState>();

    if (highstate->isTimeout()) {
    ...
    Eigen::Vector3f velocity_w;
    {
        ...
        velocity_w = Eigen::Map<const Eigen::Vector3f>(
            highstate->msg_.velocity().data()
        );
    }
    const Eigen::Vector3f velocity_b =
        env->robot->data.root_quat_w.conjugate() * velocity_w;
    return {velocity_b.x(), velocity_b.y(), velocity_b.z()};
}
