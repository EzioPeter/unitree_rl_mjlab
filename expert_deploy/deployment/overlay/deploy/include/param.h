...
inline void load_config_file()
{
    ...
    const auto bin_dir_name = bin_path.parent_path().filename().string();
    if(bin_dir_name == "bin" || bin_dir_name.rfind("build", 0) == 0)
    {
    ...
    try {
        ...
        {
            spdlog::error("config.yaml not found: {}", config_file);
            exit(1);
        }
