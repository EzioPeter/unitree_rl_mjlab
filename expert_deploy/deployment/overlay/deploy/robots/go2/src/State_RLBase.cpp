...
: FSMState(state_mode, state_string)
{
    ...
    policy_name = policy_dir.filename().string();
    ...
    if (cfg["logging"] && cfg["logging"].as<bool>()) {
        ...
        if (logging_dt.count() <= 0.0) {
            throw std::runtime_error("logging_dt must be positive");
        }
        const double logging_steps = logging_dt.count() / static_cast<double>(env->step_dt);
        if (logging_steps < 1.0 - 1.0e-6) {
            throw std::runtime_error("logging_dt must not be shorter than policy step_dt");
        }
        logging_step_interval = std::lround(logging_steps);
        if (std::abs(logging_steps - static_cast<double>(logging_step_interval)) > 1.0e-4) {
            throw std::runtime_error("logging_dt must be an integer multiple of policy step_dt");
        }
...
void State_RLBase::log_data()
{
    // Log only complete policy transitions, in the policy thread immediately
    // after observation -> inference -> ActionManager processing.
    if (enable_logging && logger) {
        auto now = std::chrono::steady_clock::now();
        if (env->episode_length % logging_step_interval == 0) {
            ...
            logger->add("policy_name", policy_name);
            logger->add("policy_state_id", std::to_string(getState()));
            logger->add("episode_step", std::to_string(env->episode_length));
            const std::vector<float>* policy_obs = nullptr;
            const auto obs_it = env->last_observations.find("obs");
            if (obs_it != env->last_observations.end()) {
                policy_obs = &obs_it->second;
            } else if (env->last_observations.size() == 1) {
                policy_obs = &env->last_observations.begin()->second;
            }
            if (policy_obs == nullptr || policy_obs->size() != 45) {
                const auto actual_size = policy_obs == nullptr ? 0 : policy_obs->size();
                throw std::runtime_error(
                    "RWM CSV logging requires exactly 45 policy observations, got "
                    + std::to_string(actual_size)
                );
            }

            const std::vector<float>* raw_policy_action = nullptr;
            const auto raw_action_it = env->last_inference_results.find("actions");
            if (raw_action_it != env->last_inference_results.end()) {
                raw_policy_action = &raw_action_it->second;
            } else if (env->last_inference_results.size() == 1) {
                raw_policy_action = &env->last_inference_results.begin()->second;
            }
            if (raw_policy_action == nullptr || raw_policy_action->size() != 12) {
                const auto actual_size = raw_policy_action == nullptr ? 0 : raw_policy_action->size();
                throw std::runtime_error(
                    "RWM CSV logging requires exactly 12 raw policy actions, got "
                    + std::to_string(actual_size)
                );
            }

            const auto effective_policy_action = env->action_manager->action();
            if (effective_policy_action.size() != 12) {
                throw std::runtime_error(
                    "RWM CSV logging requires exactly 12 effective policy actions, got "
                    + std::to_string(effective_policy_action.size())
                );
            }

            logger->add("policy_obs", *policy_obs);
            logger->add("raw_policy_action", *raw_policy_action);
            logger->add("effective_policy_action", effective_policy_action);
            logger->add(
                "unsafe_orientation",
                isaaclab::mdp::bad_orientation(env.get(), 2.0) ? 1.0f : 0.0f
            );

            const auto q_des = env->action_manager->processed_actions();
            logger->add("q_des", q_des);
