"""Visualize a V13 48D full-state Go2 FlashSAC-RWM policy in MJLab."""
...
from scripts.reinforcement_learning.rwm_dataset.broken_go2 import (  # noqa: E402
    apply_go2_broken_pd_joints,
    apply_go2_pd_joint_strength_scales,
)
from scripts.reinforcement_learning.rwm_flashsac.utils import (  # noqa: E402
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="Fixed payload mass added above the base link during MJLab play.",
    )
    parser.add_argument(
        "--rr_calf_strength",
        type=float,
        default=1.0,
        help="Fixed RR calf actuator-strength scale during MJLab play.",
    )
...
def _verify_fixed_payload_brick(env: Any, requested_mass_kg: float) -> float:
    """Fail closed unless the visible payload body has the requested mass."""
    ...
    if requested_mass_kg == 0.0:
        if matches:
            raise RuntimeError("Unexpected fixed_payload_brick in zero-payload visualization.")
        return 0.0
    if len(matches) != 1:
        raise RuntimeError(
            "Expected exactly one visible fixed_payload_brick body, "
            f"found {len(matches)} in {body_names}."
        )
...
def main() -> None:
    ...
    if not 0.0 <= rr_calf_strength <= 1.0:
        raise ValueError(
            f"rr_calf_strength must be in [0, 1], got {rr_calf_strength}."
        )
    if rr_calf_strength != 1.0:
        joint_strength_scales["RR_calf_joint"] = rr_calf_strength
    ...
    agent._actor.load(str(actor_path), load_optimizer=False)
    ...
    print(
        ...
        f"payload_mass_kg={float(args.payload_mass_kg)}, "
        f"realized_visible_payload_mass_kg={realized_payload_mass_kg}, "
        f"rr_calf_strength={rr_calf_strength}"
    )
    print(
        "[Go2-FlashSAC-RWM-Play] "
