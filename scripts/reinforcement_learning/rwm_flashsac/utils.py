...
def make_flashsac_config(cfg: Any, device: str) -> FlashSACConfig:
    ...
    actor_learning_starts_updates = int(agent_cfg.pop("actor_learning_starts_updates", 0) or 0)
    actor_learning_rate_scale = float(
        agent_cfg.pop("actor_learning_rate_scale", 1.0) or 1.0
    )
    actor_support_preserving_warmup = bool(
        agent_cfg.pop("actor_support_preserving_warmup", False)
    )
    # This option belonged to an abandoned actor-interface experiment and is
    # intentionally inert for the V12 random-initialization pilots.
    agent_cfg.pop("actor_last_action_residual_scale", None)
    ...
    flashsac_cfg = FlashSACConfig(**agent_cfg)  # type: ignore[arg-type]
    setattr(flashsac_cfg, "actor_learning_starts_updates", actor_learning_starts_updates)
    setattr(flashsac_cfg, "actor_learning_rate_scale", actor_learning_rate_scale)
    setattr(
        flashsac_cfg,
        "actor_support_preserving_warmup",
        actor_support_preserving_warmup,
    )
    return flashsac_cfg
