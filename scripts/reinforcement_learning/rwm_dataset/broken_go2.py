...
def apply_go2_broken_pd_joints(env_cfg: Any, joint_names: Sequence[str]) -> tuple[str, ...]:
    ...
    current_robot = env_cfg.scene.entities["robot"]
    spec_module = getattr(current_robot.spec_fn, "__module__", "")
    if spec_module.endswith(".unitree_mujoco_parity"):
        from src.assets.robots.unitree_go2.unitree_mujoco_parity import (
            get_go2_unitree_mujoco_robot_cfg,
        )

        env_cfg.scene.entities["robot"] = get_go2_unitree_mujoco_robot_cfg(
            broken_pd_joint_names=broken
        )
    else:
        env_cfg.scene.entities["robot"] = get_go2_robot_cfg(
            broken_pd_joint_names=broken
        )
    return broken
