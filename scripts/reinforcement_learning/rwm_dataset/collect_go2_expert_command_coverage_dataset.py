...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--expert_relabel_actions",
        action=argparse.BooleanOptionalAction,
        default=False,
        help=(
            "DAgger-style collection: execute the selected collector policy, but store the deterministic "
            "expert action for the same observation in dataset['actions']. The actual action passed to the "
            "environment is retained in dataset['raw_actions']."
        ),
    )
...
def _load_policy_actor_for_inference(agent: Any, ckpt_dir: Path) -> None:
    """Load only policy weights; replay/critic state is irrelevant to collection."""

    actor_path = ckpt_dir / "actor.pt"
    if not actor_path.is_file():
        raise FileNotFoundError(f"Policy actor checkpoint not found: {actor_path}")
    actor = getattr(agent, "_actor", None)
    if actor is None or not hasattr(actor, "load"):
        raise TypeError(
            "FlashSAC collection agent has no loadable actor network."
        )
...
def _load_expert_agent(
    ...
    *,
    ...
    if rwm_cfg_path.exists():
        ...
        _load_policy_actor_for_inference(agent, ckpt_dir)
...
def _policy_actions(
    ...
    *,
    ...
    # Ordinary expert collection uses the stock deterministic FlashSAC API,
    # which has no action_temperature keyword.  Temperature is a TRACE-only
    # extension and must not leak into the baseline collection path.
    if action_temperature is not None:
        sample_kwargs["action_temperature"] = float(action_temperature)
    actions_np = policy.agent.sample_actions(**sample_kwargs).astype(np.float32)
...
def _transition_identity_context(metadata: dict[str, Any]) -> dict[str, Any]:
    """Return fields that must match before dataset next-state identity is meaningful.

    Restoring an exact kinematic snapshot is valid across different simulator
    dynamics.  Requiring its one-step successor to equal the source dataset is
    only meaningful when the source and rollout dynamics/action interface are
    also identical.
    """
    ...
    return {
        ...
        "target_gap": metadata.get("target_gap"),
        "broken_pd_joint_names": metadata.get("broken_pd_joint_names"),
        "joint_strength_scales": metadata.get("joint_strength_scales"),
        "env_randomization": {
            key: env_randomization.get(key)
            for key in (
        ...
        "env_step_action_interface": metadata.get("env_step_action_interface"),
    }
...
def main() -> None:
    ...
    trace_foot_site_ids: torch.Tensor | None = None
    trace_diagnostics_enabled = bool(args.trace_reset_dataset) or bool(args.save_trace_snapshots)
    if trace_diagnostics_enabled:
        expected_foot_sites = ("FR", "FL", "RR", "RL")
        foot_site_ids, resolved_foot_sites = env.scene["robot"].find_sites(
            expected_foot_sites,
            preserve_order=True,
        )
        if tuple(resolved_foot_sites) != expected_foot_sites:
            raise RuntimeError(
                "Go2 TRACE foot-site order mismatch: "
                f"expected={expected_foot_sites}, resolved={tuple(resolved_foot_sites)}"
            )
        trace_foot_site_ids = torch.as_tensor(
            foot_site_ids,
            device=env.device,
            dtype=torch.long,
        )
    ...
    trace_source_metadata: dict[str, Any] | None = None
    if args.trace_reset_dataset:
        ...
        trace_source_metadata = dict(trace_source_dataset.get("metadata") or {})
    ...
    if bool(args.expert_relabel_actions) and expert_agent is None:
        raise ValueError("--expert_relabel_actions requires --expert_policy_path.")
    ...
    metadata = {
        ...
        "dagger_relabel": {
            "enabled": bool(args.expert_relabel_actions),
            "rollout_policy_path": (
                None if not bool(args.expert_relabel_actions) or args.medium_policy_path is None
                else str(args.medium_policy_path)
            ),
            "label_policy_path": (
                None if not bool(args.expert_relabel_actions) else str(args.expert_policy_path)
            ),
            "dataset_actions": (
                "expert_relabel_at_rollout_observation"
                if bool(args.expert_relabel_actions)
                else "post_interface_env_step_action"
            ),
            "raw_actions": (
                "executed_rollout_action"
                if bool(args.expert_relabel_actions)
                else "pre_interface_policy_action"
            ),
        },
        ...
        "trace_candidates": {
            ...
            "reset_reconstruction_excludes": (
                "state[33:45] actuator_force is dynamics-derived and not resettable"
    ...
    def reset_trace_candidates() -> None:
        ...
        # State[33:45] is actuator force, an output of the current dynamics,
        # not a resettable simulator coordinate.  In the intended TRACE
        # cross-dynamics rollout it must be recomputed by the nominal
        # simulator, so reset identity is defined on physical state[0:33].
        compared_width = 33
    ...
    if trace_source_states is not None:
        ...
        source_identity_context = (
            _transition_identity_context(trace_source_metadata)
            if trace_source_metadata is not None
            else None
        )
        rollout_identity_context = _transition_identity_context(metadata)
        source_dynamics_match = (
            source_identity_context is not None
            and source_identity_context == rollout_identity_context
        )
        source_match_available = (
            args.trace_reset_mode == "exact_snapshot" and source_dynamics_match
        )
        ...
        metadata["trace_candidates"]["source_transition_identity"] = {
            ...
            "max_state_abs_error": source_next_max_error,
            "tolerance": (
                float(args.trace_identity_tolerance) if source_match_available else None
            ),
            ...
            "unavailable_reason": (
                ...
                if source_match_available
                    ...
                    "source and rollout dynamics/action interface differ by design"
                    if args.trace_reset_mode == "exact_snapshot"
                    else "real logs do not contain an exact simulator snapshot"
                )
            ),
            "source_dynamics_match": bool(source_dynamics_match),
            "source_context": source_identity_context,
            "rollout_context": rollout_identity_context,
        }
        reset_trace_candidates()
    ...
    for collection_step in tqdm.trange(total_steps, smoothing=0.1, mininterval=0.5):
        ...
        transition_foot_site_position_w = (
            env.scene["robot"].data.site_pos_w[:, trace_foot_site_ids].clone()
            if trace_foot_site_ids is not None
            else None
        )
        transition_foot_site_linear_velocity_w = (
            env.scene["robot"].data.site_lin_vel_w[:, trace_foot_site_ids].clone()
            if trace_foot_site_ids is not None
            else None
        )
        ...
        label_action_t = action_t
        if bool(args.expert_relabel_actions):
            expert_label_t = _policy_actions(expert_agent, observations_by_kind)
            if expert_label_t is None:
                raise RuntimeError("Expert relabeling was requested, but the expert policy produced no action.")
            label_action_t = expert_label_t.to(device=env.device, dtype=torch.float32).clamp(-1.0, 1.0)
        ...
        add_kwargs = dict(
            ...
            next_obs=next_obs_t,
            state=state,
            action=label_action_t,
            raw_action=env_action_t if bool(args.expert_relabel_actions) else action_t,
        ...
        # TRACE diagnostics and simulator snapshots belong to the extended
        # builder interface.  Do not pass them to the stock baseline builder
        # during ordinary expert collection.
        if trace_source_states is not None or args.save_trace_snapshots:
            extended = {
                ...
                "trace_foot_site_position_w": transition_foot_site_position_w,
                "trace_foot_site_linear_velocity_w": transition_foot_site_linear_velocity_w,
            }
            unsupported = set(extended).difference(
                inspect.signature(builder.add).parameters
            )
            if unsupported:
                raise RuntimeError(
                    "TRACE/snapshot collection requires an extended dataset "
                    f"builder; unsupported fields: {sorted(unsupported)}"
                )
            add_kwargs.update(extended)
        builder.add(**add_kwargs)
