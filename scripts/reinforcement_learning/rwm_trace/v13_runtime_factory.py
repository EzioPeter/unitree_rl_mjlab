...
def create_v13_online_trace_manager(
    *,
    ...
    raw_command_region_sim_ratios = OmegaConf.select(
        cfg, "trace.command_region_sim_ratios", default=None
    )
    command_region_sim_ratios = (
        None
        if raw_command_region_sim_ratios is None
        else {
            str(region): float(ratio)
            for region, ratio in OmegaConf.to_container(
                raw_command_region_sim_ratios, resolve=True
            ).items()
        }
    )
    manager = Go2OnlineTraceManager(
        config=OnlineTraceConfig(
            ...
            * 3,
            ...
            command_region_sim_ratios=command_region_sim_ratios,
        ),
