...
V13 active dataset V1 snapshots + current FlashSAC actor
  ...
  -> Stage 2: fixed same/cross LLM pairs + per-dataset BT scorer
  -> one global top-alpha (rule score or final learned score)
...
| `feedback_pairs.py` | 不读 score/return；按固定 same/cross quota、command mode/magnitude bin 配对。 |
...
| `selection.py` | 对当次全部 valid candidates 做一次 `ceil(alpha*N)` global top-alpha。 |
