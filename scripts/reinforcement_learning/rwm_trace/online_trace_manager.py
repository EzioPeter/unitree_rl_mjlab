...
class Go2OnlineTraceManager:
    def __init__(
        ...
        *,
        ...
        self.buffer = MutableTraceReplayBuffer(
            config.buffer_capacity, seed=config.proposal_seed + 2
        )
    ...
    def maybe_propose(
        ...
        selected_region_counts = Counter(
            command_region(summaries[index], planar_scales)
        ...
        self.last_report = {
            ...
            "candidate_command_region_counts": dict(
                sorted(candidate_region_counts.items())
            ),
