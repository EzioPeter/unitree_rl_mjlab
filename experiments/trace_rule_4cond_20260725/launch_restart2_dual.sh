...
output_root="/data1/xjy/unitree_rl_mjlab_v13/logs/experiments/go2_v13_rule_trace_T4_a010_r010_nosatgate_from_scratch_update97458_seed0_20260726_restart2"
log_root="${experiment_root}/logs_restart2"
session_root="${experiment_root}/sessions_restart2"
...
launch_one() {
  ...
  local session_name="trace_rule_restart2_${condition}_gpu${physical_gpu}"
  ...
  if [[ -e "${output}" || -e "${log}" || -e "${pid_file}" || -e "${session_file}" ]]; then
    echo "Refusing to overwrite restart2 artifacts for ${condition}" >&2
    return 1
  fi
